...
export abstract class Madara extends Source {
     ...
     * which is looped over. This may be overridden if required.
     */
    chapterDetailsSelector = '.page-break.no-gaps > img';
    ...
    override async getHomePageSections(sectionCallback: (section: HomeSection) => void): Promise<void> {
        const json = await this.getJsonData(this, 'https://pandeynmn.github.io/rs.html')
        const sections = [
            {
            ...
            },
            {
                ...
                section: createHomeSection({
                    id: 'dev-titles',
                    title: json.heading ?? 'Error Github',
                    view_more: false,
            ...
            },
            {
                ...
                section: createHomeSection({
                    id: 'coming-soon',
                    title: 'Coming Soon' ?? 'Error Github',
                    view_more: false,
    ...
    override async getViewMoreItems(homepageSectionId: string, metadata: any): Promise<PagedResults> {
        ...
        this.CloudFlareError(data.status)
        const $ = this.cheerio.load(data.data)
        const items: MangaTile[] = this.parser.parseHomeSection($, this, homepageSectionId)
    ...
    async getJsonData(object: Source, url: string) {
        const request = createRequestObject({
            url,
            method: 'GET',
        })
        const response = await object.requestManager.schedule(request, 1)
