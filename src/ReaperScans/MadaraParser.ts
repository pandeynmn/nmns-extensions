...
export class Parser {
    ...
    parseMangaDetails($: CheerioSelector, mangaId: string): Manga {
        ...
        if (!numericId) throw new Error(`Could not parse out the data-id for ${mangaId} - This method might need overridden in the implementing source`)
        ...
        if (!image) throw new Error(`Could not parse out a valid image while parsing manga details for manga: ${mangaId}`)
    ...
    parseChapterList($: CheerioSelector, mangaId: string, source: any): Chapter[] {
        ...
        for (const obj of $('li.wp-manga-chapter').toArray()) {
            const id = ($('a', $(obj)).first().attr('href') || '').split('/')[5] ?? ''
    ...
    parseSearchResults($: CheerioSelector, source: any): MangaTile[] {
        ...
        for (const obj of $(source.searchMangaSelector).toArray()) {
            const id = ($('a', $(obj)).attr('href') ?? '').split('/')[4] ?? ''
            ...
            if (this._novelArr.includes(id)) continue
            results.push(
                createMangaTile({
                    id: id,
                    title: title,
    ...
    parseHomeSection($: CheerioStatic, source: any, sectionId: string): MangaTile[] {
        const items: MangaTile[] = []
        if (sectionId == 'dev-titles') return this._githubTitleParse($)  
        if (sectionId == 'coming-soon') return this._comingSoon()
    ...
    _githubTitleParse($: CheerioStatic, ) {
        const items: MangaTile[] = []
        ...
        for (const obj of json.seriesList) {
            items.push(
                createMangaTile({
                    id: obj.id ?? 'error',
                    image: obj.image ?? '',
                    title: createIconText({ text: obj.title ?? 'error' }),
                    subtitleText: createIconText({ text: obj.subtitleText ?? 'error' }),
                })
            )
        }
        return items
